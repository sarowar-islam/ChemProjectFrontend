  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="font-heading text-2xl md:text-3xl text-primary">{title}</h2>
...
function MemberCard({ member, delay, featured }: { member: TeamMember; delay: string; featured?: boolean }) {
  return (
    <Link
      ...
      className={`group relative bg-card rounded-xl sm:rounded-2xl border border-border overflow-hidden hover:shadow-elevated transition-all duration-300 animate-fade-in-up ${featured ? 'p-4 sm:p-8' : 'p-4 sm:p-6'}`}
      style={{ animationDelay: delay }}
    >
      <div className="absolute top-0 left-0 right-0 h-16 sm:h-24 bg-gradient-to-b from-primary/5 to-transparent" />
      
      <div className="relative text-center">
        <div className={`relative mx-auto mb-3 sm:mb-4 overflow-hidden rounded-full border-2 sm:border-4 border-background shadow-lg group-hover:border-accent transition-colors ${featured ? 'w-24 h-24 sm:w-36 sm:h-36' : 'w-20 h-20 sm:w-28 sm:h-28'}`}>
          <img
            src={member.photoUrl}
            alt={member.name}
        ...
        <h3 className={`font-heading font-semibold text-foreground group-hover:text-primary transition-colors ${featured ? 'text-base sm:text-xl' : 'text-sm sm:text-lg'}`}>
          {member.name}
        </h3>
        <p className="text-xs sm:text-sm text-muted-foreground mt-1">{member.designation}</p>
        
        {member.researchArea && (
          <div className="mt-2 sm:mt-3 inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-accent/10 text-accent text-[10px] sm:text-xs font-medium">
            {member.researchArea}
          </div>
        )}
        ...
        {member.expertise && member.expertise.length > 0 && (
          <div className="mt-2 flex flex-wrap justify-center gap-1">
            {member.expertise.slice(0, 2).map((exp) => (
              <span
                key={exp}
                className="text-[9px] sm:text-[10px] px-1.5 py-0.5 bg-primary/5 text-primary/70 rounded"
              >
                {exp}
              </span>
            ))}
        ...
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border">
          <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs text-muted-foreground group-hover:text-primary transition-colors">
