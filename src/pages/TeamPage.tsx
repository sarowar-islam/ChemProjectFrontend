      ...
      <section className="relative py-16 sm:py-20 md:py-28 overflow-hidden">
        ...
        <div className="container-wide relative">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <Link
              to="/teamlogin"
              className="group inline-flex items-center gap-2.5 px-6 py-3 bg-[#38BDF8] text-[#0B1220] rounded-xl text-sm font-semibold shadow-lg hover:bg-[#0EA5E9] transition-all duration-300 hover:-translate-y-0.5 animate-fade-in-up"
      ...
      <section className="container-wide -mt-8 sm:-mt-10 relative z-10 mb-12 sm:mb-16">
        <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-2xl mx-auto">
          <div className="bg-card rounded-xl sm:rounded-xl p-3 sm:p-6 shadow-card border border-border text-center animate-fade-in-up">
            <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-[#38BDF8] text-[#0B1220] flex items-center justify-center mx-auto mb-2 sm:mb-3">
              <GraduationCap className="w-4 h-4 sm:w-6 sm:h-6" />
            </div>
            <div className="font-heading text-lg sm:text-2xl font-bold text-[#38BDF8]">{teamLeaders.length + faculty.length}</div>
          ...
          <div className="bg-card rounded-xl sm:rounded-xl p-3 sm:p-6 shadow-card border border-border text-center animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="font-heading text-lg sm:text-2xl font-bold text-[#38BDF8]">{researchers.length}</div>
          ...
          <div className="bg-card rounded-xl sm:rounded-xl p-3 sm:p-6 shadow-card border border-border text-center animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="font-heading text-lg sm:text-2xl font-bold text-[#38BDF8]">{students.length}</div>
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <div className="p-3 rounded-xl bg-[#38BDF8]/15 text-[#38BDF8]">
          {icon}
        </div>
        <div>
...
function MemberCard({ member, delay, featured }: { member: TeamMember; delay: string; featured?: boolean }) {
  return (
    <Link
      ...
      className={`group relative bg-card rounded-xl sm:rounded-2xl border border-border overflow-hidden hover:shadow-elevated hover:border-[#38BDF8]/25 transition-all duration-300 animate-fade-in-up ${featured ? 'p-4 sm:p-8' : 'p-4 sm:p-6'}`}
      style={{ animationDelay: delay }}
    >
      <div className="absolute top-0 left-0 right-0 h-16 sm:h-24 bg-gradient-to-b from-[#38BDF8]/5 to-transparent" />
      
      <div className="relative text-center">
        <div className={`relative mx-auto mb-3 sm:mb-4 overflow-hidden rounded-full border-2 sm:border-4 border-background shadow-lg group-hover:border-[#38BDF8] transition-colors ${featured ? 'w-24 h-24 sm:w-36 sm:h-36' : 'w-20 h-20 sm:w-28 sm:h-28'}`}>
          <img
            src={member.photoUrl}
            alt={member.name}
        ...
        <h3 className={`font-heading font-semibold text-foreground group-hover:text-[#38BDF8] transition-colors ${featured ? 'text-base sm:text-xl' : 'text-sm sm:text-lg'}`}>
          {member.name}
        </h3>
        <p className="text-xs sm:text-sm text-muted-foreground mt-1">{member.title}</p>
        
        {member.researchArea && (
          <div className="mt-2 sm:mt-3 inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-[#38BDF8]/15 text-[#38BDF8] text-[10px] sm:text-xs font-medium">
            {member.researchArea}
          </div>
        )}
        ...
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border">
          <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs text-muted-foreground group-hover:text-[#38BDF8] transition-colors">
